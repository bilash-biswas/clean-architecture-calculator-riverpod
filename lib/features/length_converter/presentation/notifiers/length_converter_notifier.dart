...
import '../../data/models/length_history_model.dart';
import '../../domain/usecases/convert_length.dart';

class LengthConverterState {
  final String fromUnit;
  final String toUnit;
  final String fromValue;
  final String toValue;
  final bool isFromFocused;
  final List<LengthHistoryModel> history;

  LengthConverterState({
    this.fromUnit = 'm',
    this.toUnit = 'cm',
    this.fromValue = '1',
    this.toValue = '100',
  ...
  LengthConverterState copyWith({
    String? fromUnit,
    String? toUnit,
    String? fromValue,
    String? toValue,
    bool? isFromFocused,
    List<LengthHistoryModel>? history,
  }) {
    return LengthConverterState(
...
class LengthConverterNotifier extends Notifier<LengthConverterState> {
  late final Box<LengthHistoryModel> _box;
  final _convertLength = ConvertLength();

  @override
  LengthConverterState build() {
    _box = Hive.box<LengthHistoryModel>(DatabaseService.lengthHistoryBoxName);
    final list = _box.values.toList();
    list.sort((a, b) => b.timestamp.compareTo(a.timestamp));
    return LengthConverterState(history: list);
  }
  ...
  void _updateTargetAndConvert(String text) {
    ...
    final convertedVal = _convertLength(
      value: numericVal,
  ...
  void setFromUnit(String unit) {
    ...
    if (state.isFromFocused) {
      ...
      final convertedFrom = _convertLength(
        value: numericTo,
  ...
  void setToUnit(String unit) {
    ...
    if (!state.isFromFocused) {
      ...
      final convertedTo = _convertLength(
        value: numericFrom,
  ...
  Future<void> saveToHistory() async {
    ...
    final model = LengthHistoryModel.fromValues(
      id: 0,
...
final lengthConverterProvider =
    NotifierProvider<LengthConverterNotifier, LengthConverterState>(LengthConverterNotifier.new);
